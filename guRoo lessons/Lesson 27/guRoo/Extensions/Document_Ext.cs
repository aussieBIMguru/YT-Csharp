...
namespace guRoo.Extensions
{
    public static class Document_Ext
    {
        ...
        public static List<Element> Ext_ElementsOfCategory(this Document doc, BuiltInCategory builtInCategory, View view = null)
        {
            if (doc is null) { return new List<Element>(); }

            return doc.Ext_Collector(view: view)
        ...
        /// <returns></returns>
        public static List<FamilySymbol> Ext_TitleBlockTypes(this Document doc, bool sorted = true)
        {
            // Collect our family types
            var ttbTypes = doc.Ext_Collector()
            ...
            {
                return ttbTypes
                    .OrderBy(f => f.Ext_ToFamilySymbolKey())
                    .ToList();
            }
            else
            {
        ...
        /// <param name="sorted"></param>
        /// <returns></returns>
        public static gFrm.FormResult<FamilySymbol> Ext_SelectTitleBlockTypes(this Document doc, string title = null,
            bool multiSelect = true, bool sorted = true)
        {
            // Default title
            title ??= multiSelect ? "Select family type(s) from the list:" : "Select a family type from the list";

            // Get family types
            var ttbTypes = doc.Ext_TitleBlockTypes(sorted);

            // Process into keys and values
            var keys = ttbTypes
                .Select(f => f.Ext_ToFamilySymbolKey())
                .ToList();

            // Return the form outcome
            return gFrm.Custom.SelectFromList<FamilySymbol>(keys, ttbTypes, title, multiSelect);
        }
