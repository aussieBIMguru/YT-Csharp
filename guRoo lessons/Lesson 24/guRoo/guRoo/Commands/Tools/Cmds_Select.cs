...
using gSel = guRoo.Utilities.Selection_Utils;
...
namespace guRoo.Cmds_Select
{
    ...
    public class Cmd_PickRooms : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            // Make the selection filter
            var selectionFilter = new gSel.ISF_ByCategory(BuiltInCategory.OST_Rooms);

            // Pick the rooms
            var selectedElements = uiDoc.Ext_SelectWithFilter(selectionFilter,
                prompt: "Select room(s), then press \'Finish\'");

            // Make sure we got some rooms
            if (selectedElements.Count == 0)
            {
                return gFrm.Custom.Cancelled("No elements selected.");
            }
    ...
    /// </summary>
    [Transaction(TransactionMode.Manual)]
    public class Cmd_PickHidden : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            // Collect the document
            ...
            // Make the selection filter
            var selectionFilter = new gSel.ISF_HiddenInView(activeView);

            // Pick the elements
            var selectedElements = uiDoc.Ext_SelectWithFilter(selectionFilter,
                prompt: "Select element(s), then press \'Finish\'");

            // Make sure we got some elements
            if (selectedElements.Count == 0)
            {
                return gFrm.Custom.Cancelled("No elements selected.");
            }
